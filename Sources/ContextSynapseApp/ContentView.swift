        ...
        HStack(spacing: 16) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                GroupBox("Fault Injection & Resilience") {
                    VStack(alignment: .leading) {
                        Toggle(isOn: Binding(
                            get: { vm.faultEnabled },
                            set: { vm.setFaultEnabled($0) }
                        )) {
                            Text("Enable fault injection simulation")
                        }
                        HStack {
                            Slider(value: Binding(
                                get: { vm.faultProbability },
                                set: { vm.setFaultProbability($0) }
                            ), in: 0.0...0.9)
                            .disabled(!vm.faultEnabled)
                            Text(String(format: "%.2f", vm.faultProbability)).frame(width: 50)
                        }
                        HStack {
                            Button("Disintegrate Sky Plates") { vm.disintegrateSkyPlates() }
                            Text("(safe simulated corruption for resilience testing)")
                                .font(.caption)
            ...
            VStack(spacing: 12) {
                ...
                GroupBox("Nearest Regions") {
                    VStack(alignment: .leading) {
                        ForEach(vm.regions, id: \.name) { region in
                            HStack {
                                Text(region.name).bold()
                                Spacer()
                                if let (nearest, score) = vm.nearestMap[region.name] {
                                    Text("\(nearest): \(String(format: "%.3f", score))")
                                } else {
                                    Text("-")
                                }
                            }
