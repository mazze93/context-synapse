...
public struct Prior: Sendable, Codable, Equatable {
    ...
    /// Expected utility: E[Prior] = α / (α + β). Range: (0, 1).
    ...
    public static let uninformed = Prior(alpha: 1.0, beta: 1.0)
    ...
    public static func lighthouse(confidence: Double = 10.0) -> Prior {
        Prior(alpha: confidence * 0.8, beta: confidence * 0.2)
    }
...
public struct SynapticNode: Identifiable, Sendable, Codable, Equatable {
    ...
    public init(id: UUID = UUID(), synapseID: String, prior: Prior = .uninformed) {
        self.id        = id
